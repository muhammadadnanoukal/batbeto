...
class ApprovalRequest(models.Model):
    ...
    current_approver = fields.Many2one('approval.approver', compute='compute_current_approver', store=True)
    notified_user = fields.Many2many('res.users')

    # @api.depends('approver_ids.status')
    # def compute_notified_user(self):
    #
    ...
    @api.depends('request_owner_id', 'name')
    ...
    def _compute_has_access_to_request(self):
        ...
        for request in self:
            ...
            print('request.has_access_to_request', request.has_access_to_request)
    ...
    def action_confirm(self):
        ...
        print('approvers ids', approvers.ids)
        users_ids = []
        for approver in approvers:
            users_ids.append(approver.user_id.id)
        print('users ids', users_ids)
        self.notified_user = [(6, 0, users_ids)]
        print('notified users', self.notified_user)
    ...
    def _update_next_approvers(self, new_status, approver, only_next_approver, cancel_activities=False):
        ...
        if new_status == 'pending':
            ...
            print('app')
            # self.notified_user = [(6, 0, approvers_updated.ids)]
            print('notified user 1', self.notified_user)
        if cancel_activities:
            self.notified_user = [(6, 0, [])]
            approvers_updated.request_id._cancel_activities()
            print('notified user 2', self.notified_user)
    ...
    def action_approve(self, approver=None):
        ...
        y = self.sudo()._get_user_approval_activities(user=self.env.user).action_feedback()
        print('y   y', y)

    def action_refuse(self, approver=None):
        print('approver in refuse', approver)
        if not isinstance(approver, models.BaseModel):
            approver = self.mapped('approver_ids').filtered(
                lambda approver: approver.user_id == self.env.user
            )
        ...
        x = self.sudo()._get_user_approval_activities(user=self.env.user).action_feedback()
        print('x', x)
        # if self.category_id.notify_even_when_refused:
        #     print('notify even when refused ? ', self.category_id.notify_even_when_refused)
        #     approvers_updated = self.approver_ids
        #     print('next user id ', approvers_updated)
        #     self.sudo()._get_user_approval_activities(user=approvers_updated[0].user_id).action_feedback()
        #     pass
    ...
    def _compute_user_status(self):
        for approval in self:
            ...
            print('approver status', approval.user_status)
...
class ApprovalApprover(models.Model):
    ...
    # def _create_refuse_activity(self):
    #     self.env['mail.activity'].create({
    #         'display_name': 'The request were refused',
    #         'summary': 'text',
    #         'user_id': self.user_id.id,
    #         'res_id': 'approval.request',
